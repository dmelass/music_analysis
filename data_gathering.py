...
# remove anything after '(', inclusive
def remove_paran(name):
    paran_loc = name.index('(')
    name = name[:paran_loc].strip()
    return name
...
# some song names have numbers to represent arabic letters not in english
...
with open('artists_and_songs.pkl','w') as picklefile:
    pickle.dump(artists_and_songs,picklefile)
    
with open('artists_and_songs_id.pkl','w') as picklefile:
    pickle.dump(artists_and_songs_id,picklefile)
...
arabic_songs = load_pickle('all_arabic_songs.pkl') #3515 of them

# steps for getting arabic songs:
# 1. search for song with artist name
# 2. if exists, get echonest ID and put in dictionary with same keys
# 3. and add 1 to count
# 4. if doesn't exist, output to log
# 5. at the end, output count of found songs

# import sys
# sys.exit()

count = 0
audio_summaries_arabic = {}
arabic_song_ids = {}
not_found = []
for info in arabic_songs.keys(): #for each artist/song combo
    artist_name = info[0]
    if artist_name == 'Mohammed Hamaki':
        artist_name = 'Mohamed Hamaki'
    if artist_name == 'Majida El Roumi':
        artist_name = 'Majda El Roumi'
    if artist_name == 'George Al-Rasi':
        artist_name = 'Geroge Al Rassi'
    orig_song_name = info[1]
    song_name = orig_song_name
    if '(' in song_name:
        song_name = remove_paran(song_name)
    if '-' in song_name:
        song_name = remove_dashes(song_name)
    if any(c.isdigit() for c in song_name):
        song_name = remove_numbers(song_name)
    translated_song_name = info[2]
    try:
        echo_output = song.search(artist=artist_name, title=song_name)
        if len(echo_output) != 0: #if exists
            count += 1
            song_id = echo_output[0].id
            song_audio_summary = echo_output[0].audio_summary
            arabic_song_ids[info] = song_id
            audio_summaries_arabic[info] = song_audio_summary
            print 'found ' + song_name + ' by ' + artist_name + ' (orig name:) ' + orig_song_name
        else:
            print 'could not find ' + song_name + ' by ' + artist_name + ' (orig name:) ' + orig_song_name
...
create_pickle(audio_summaries_arabic, 'arabic_audio_summaries.pkl')
create_pickle(arabic_song_ids, 'arabic_song_ids.pkl')
create_pickle(not_found, 'not_found_songs.pkl')

print 'found ' + str(count) + ' of 3515 songs on echonest'
